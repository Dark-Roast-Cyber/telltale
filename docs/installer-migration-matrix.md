...
| Canonical effective policy | Unit-specific or ambiguous drop-ins fail closed. Type-wide `service.d`/`timer.d` candidates are allowed only when the effective canonical service contract and exact two-entry timer schedule remain unchanged. |
...
regular-file type, path safety, ownership, mode, canonical unit fragment,
structural drop-in scope, and effective-unit safety. Inherited type-wide policy
is accepted only when the canonical effective contract remains unchanged. The
journal records only bounded
...
and optional timer activation. Retained recovery first quiesces stale canonical
schedules, preserves canonical state bytes, and removes only proven generated
transaction bytes. A failed check leaves unrelated resources alone;
