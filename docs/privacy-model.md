...
delivery, or manifest-file writes. Embedded process-chain, source-adapter, MCP,
...
**Current event behavior**: Telltale does not emit a separate local-only context field. Raw source-native records and direct-record detector inputs remain local process memory only; emitted canonical Events carry bounded sanitized excerpts and approved metadata.
