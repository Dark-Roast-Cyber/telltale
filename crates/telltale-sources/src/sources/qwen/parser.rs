...
pub(crate) fn extract_qwen_jsonl_source(
    source: &Source,
    ...
    for value in values {
        ...
        records.push(ParsedRecord {
            ...
            kind: qwen_record_kind(&value),
            tool_name: qwen_tool_name(&value),
            arguments: arguments_field(&value).or_else(|| qwen_tool_input_as_string(&value)),
...
fn qwen_record_kind(value: &Value) -> RecordKind {
...
fn qwen_discriminator(value: &Value) -> Option<&str> {
    value
        .get("payload")
...
fn is_known_qwen_discriminator(kind: &str) -> bool {
    matches!(
        kind,
        "user_message"
            | "user"
...
fn qwen_tool_part_is_result(value: &Value) -> bool {
    value
        .get("state")
...
fn qwen_tool_name(value: &Value) -> Option<String> {
...
fn qwen_tool_input_as_string(value: &Value) -> Option<String> {
